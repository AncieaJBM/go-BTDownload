...
const (
	MsgChoke       MsgId = 0 //不上传
	MsgUnchoke     MsgId = 1 //上传
	MsgInterested  MsgId = 2 //下载
	MsgNotInterest MsgId = 3 //不下载
	MsgHave        MsgId = 4 //连接中 新的piece
	MsgBitfield    MsgId = 5 //位图
	MsgRequest     MsgId = 6 //下载请求 (piece - startByte - endByte)
	MsgPiece       MsgId = 7 //下载内容
	MsgCancel      MsgId = 8
)
...
func (c *PeerConn) ReadMsg() (*PeerMsg, error) {
	//PeerMsg的格式为4个字节标识这个msg大小（不包括自己）然后一个字节标识类型，后面就是内容。
	//如果是MsgHave的话，那么后面就是4个字节，标识自己拥有的piece的index
	/**
	+---------------+
